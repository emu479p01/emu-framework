...
export const THAI_REPORT_FONT = 'Noto Sans Thai';
const root = join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..');
const cacheDir = () => process.env.EMU_FONT_DIR ?? (process.env.EMU_DEPLOYMENT_MODE === 'docker' ? '/data/fonts' : join(root, 'fonts'));
const bundledThaiDir = join(root, 'packages', 'server', 'assets', 'fonts', 'NotoSansThai');
const bundledThaiVariants: Record<string, string> = {
  regular: join(bundledThaiDir, 'NotoSansThai-Regular.ttf'),
  '700': join(bundledThaiDir, 'NotoSansThai-Bold.ttf'),
  italic: join(bundledThaiDir, 'NotoSansThai-Regular.ttf'),
...
export function registerPdfFonts(kernel: Kernel): Set<string> {
  const definitions: Record<string, { normal: string; bold: string; italics: string; bolditalics: string }> = {
    ...
    [THAI_REPORT_FONT]: {
      ...
      bold: bundledThaiVariants['700'],
      italics: bundledThaiVariants.italic,
      bolditalics: bundledThaiVariants['700italic'],
    },
  };
...
export function registerFontRoutes(app: FastifyInstance, kernel: Kernel, requireAdmin: (req: FastifyRequest) => string, requireUser: (req: FastifyRequest) => unknown): void {
  ...
  app.get('/api/fonts', (req) => { requireUser(req); return { defaultFont: DEFAULT_REPORT_FONT, fonts: [{ family: DEFAULT_REPORT_FONT, builtIn: true }, { family: THAI_REPORT_FONT, builtIn: true, subsets: ['latin', 'thai'], variants: { regular: 'regular', '700': '700', italic: 'italic', '700italic': '700italic' } }, ...configuredFonts(kernel).map((font) => ({ ...font, builtIn: false }))] }; });
  ...
  app.get<{ Params: { family: string; variant: string } }>('/api/fonts/:family/:variant', async (req, reply) => {
    ...
    if (familyName === THAI_REPORT_FONT) {
      const bundled = bundledThaiVariants[req.params.variant]
        ?? bundledThaiVariants[req.params.variant.includes('italic') ? 'italic' : req.params.variant === '700' ? '700' : 'regular'];
      reply.header('Content-Type', 'font/ttf');
      reply.header('Cache-Control', 'private, max-age=86400');
      return reply.send(await readFile(bundled));
    }
    const family = configuredFonts(kernel).find((font) => font.family === familyName);
    const relative = family?.variants[req.params.variant] ?? family?.variants[req.params.variant.includes('italic') ? 'italic' : req.params.variant === '700' ? '700' : 'regular'] ?? family?.variants.regular;
    if (!relative) return reply.status(404).send({ error: 'Font variant not found' });
    reply.header('Content-Type', 'font/ttf');
    reply.header('Cache-Control', 'private, max-age=86400');
    return reply.send(await readFile(join(cacheDir(), relative)));
  });
}
