...
describe('system maintenance', () => {
  ...
  it('reports the latest stable release without accepting a client-selected version', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(new Response(JSON.stringify({
      tag_name: '0.1.6.0', name: 'Stable', body: 'Release notes', html_url: 'https://example.test/release',
    ...
    expect(response.json()).toMatchObject({ currentVersion: '0.1.6.0', latestVersion: '0.1.6.0', updateAvailable: false });
  });
