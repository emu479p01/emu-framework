...
<template>
  <div class="font-page">
    <n-space justify="space-between" align="center"><div><h1>Report Fonts</h1><p>Install Google Fonts once, then use them offline in Report Designer and PDFs.</p></div><n-button @click="showSettings = true">API settings {{ maskedKey }}</n-button></n-space>
...
<style scoped>.font-page{max-width:1100px;margin:0 auto}.font-page h1{margin:0}.font-page p{color:var(--emu-muted)}.card{margin-top:18px}.catalog{margin-top:18px;display:grid;grid-template-columns:1fr 1fr;gap:10px}.font-row{display:flex;justify-content:space-between;align-items:center;border:1px solid var(--emu-border);border-radius:9px;padding:12px}.font-row small{display:block;color:var(--emu-muted);margin-top:4px}@media(max-width:700px){.catalog{grid-template-columns:1fr}}</style>
