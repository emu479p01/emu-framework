...
17. [Web Designer (Customize บนเว็บ)](#17-web-designer-customize-บนเว็บ)
...
เมื่อใช้ `pnf nf add app` — ระบบจะ:
...
ผ่าน SystemUserRole table (จัดการผ่านหน้า Users ใน UI):
...
- Assign Role ให้ User ผ่าน SystemUserRole
...
## 17. Web Designer (Customize บนเว็บ)

ผู้ใช้ role **Admin / SystemAdminRole** จะเห็นเมนู **Development → Designer** ใน sidebar — ใช้สร้างและแก้ metadata ได้จากเว็บโดยตรง **มีผลทันที ไม่ต้อง restart server**

### ทำอะไรได้บ้าง

| การกระทำ | ผลลัพธ์ |
|---|---|
| **New → Table** | สร้าง table ใหม่ (field grid ครบทุก type รวม enum/reference) + ติ๊กให้สร้าง form และเพิ่มเข้าเมนู "Custom" อัตโนมัติ |
| **New → Enum / Form / Menu** | สร้าง artifact แต่ละชนิดด้วย editor เฉพาะ |
| **Customize existing tables** | เพิ่ม field ให้ table ที่มาจากไฟล์ app (สร้างเป็น `tableExtension` ชั้น web) |
| แท็บ **JSON** | แก้ raw JSON ตรงๆ สำหรับ property ที่ editor ยังไม่มี (เช่น `lines`, `actions`, `indexes`) |

### กลไกภายใน

- Artifact ที่สร้างจากเว็บถูกเก็บใน DB (ตาราง `SystemWebArtifact`) เป็น **web layer** — app อีกชั้นชื่อ `web` ที่ merge ทับ file apps ตอน boot และทุกครั้งที่ save
- ทุกครั้งที่ save ระบบ **rebuild registry ทั้งชุด + validate** — artifact ที่ผิด (เช่นอ้าง table ที่ไม่มี) จะถูกปฏิเสธ (422) โดยของเดิมไม่พัง แล้ว sync schema ลง SQLite ทันที
- API: `GET/PUT/DELETE /api/designer/artifacts[/:kind/:name]` (เฉพาะ admin)
...
โดยไม่ต้อง restart server ทุกอย่างเก็บใน `SystemWebArtifact` table และ apply ทันที

### 17.1 การเข้าใช้งาน

- Login ด้วย `admin` (หรือ user ที่มี `Admin`/`SystemAdminRole`)
...
2. เลือก **App** (sidebar group) — เลือกว่าจะให้ table นี้อยู่ภายใต้ app ไหน
   - เลือก app ที่มีอยู่: `Mini ERP`, `HR`, ...
